...
tidy(model1_avg_BMI_cat, exponentiate = TRUE, conf.int = TRUE) 

model2_avg_BMI_cat <- 
  glm(death2018 ~ `9age_y` + female + hispanic + black + other + 
          as.factor(avg_BMI_cat) + smoker + , family = poisson(link = "log"), 
      data = hrs_samp)
  
tidy(model2_avg_BMI_cat, exponentiate = TRUE, conf.int = TRUE) 
```