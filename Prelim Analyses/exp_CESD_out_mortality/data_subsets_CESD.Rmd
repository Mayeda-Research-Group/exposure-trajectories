...
hrs_samp %<>% 
  ...
  mutate("elevated_depress_sx_med" = ifelse(med_CESD > 3, 1, 0), 
         "elevated_depress_sx_last" = ifelse(r9cesd > 3, 1, 0))
...
model1_elevated_depress_sx_last <- 
  glm(death2018 ~ `9age_y` + female + hispanic + black + other + 
          elevated_depress_sx_last, family = poisson(link = "log"), 
      data = hrs_samp)
  
kable(tidy(model1_elevated_depress_sx_last, exponentiate = TRUE, 
           conf.int = TRUE)) %>% kableExtra::kable_styling()
```
...
## Time-to-Event
These analyses are more in line with what researchers would actually conduct.
