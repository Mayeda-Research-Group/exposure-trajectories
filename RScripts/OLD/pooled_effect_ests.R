pooled_estimates <- 
  function(data_wide, mechanism, mask_props, num_impute, methods){
    #---- create imputed datasets ----
    mask_impute(data_wide = data_wide, mechanism = mechanism, 
                mask_props = mask_props, num_impute = num_impute, save = "no")
    ...
    model_list <- 
      lapply(model_list <- vector(mode = "list", length(methods)),
             function(x) x <- 
               lapply(x <- vector(mode = "list", length(mask_props)), 
                      function(x) x <- 
