...
analytic_df_birth_cohort <- 
  read_csv(here::here("Data", "analytic_df_1931-1941_cohort.csv"))
analytic_df_alive_70 <- 
  read_csv(here::here("Data", "analytic_df_alive_70.csv"))
```
...
CYSC_bywave <- analytic_df %>% dplyr::select(contains("CYSC_ADJ")) %>% 
  ...
  mutate("Proportion of <br>2016 HRS sample" = 
  ...
  dplyr::select("Wave", "Year", "Count", "Proportion of <br>2016 HRS sample")
...
analytic_df_birth_cohort[, "num_measures"] <- 5 - (
  analytic_df_birth_cohort %>% 
  dplyr::select(contains("CYSC_ADJ")) %>% is.na(.) %>% rowSums())

analytic_df_birth_cohort$num_measures <- 
  as.factor(analytic_df_birth_cohort$num_measures)

analytic_df_alive_70[, "num_measures"] <- 5 - (
  analytic_df_alive_70 %>% 
  dplyr::select(contains("CYSC_ADJ")) %>% is.na(.) %>% rowSums())

analytic_df_alive_70$num_measures <- 
  as.factor(analytic_df_alive_70$num_measures)
...
Number of people: `r nrow(analytic_df)`
Number of observations: `r sum(as.numeric(analytic_df$num_measures))`
Percent mortality: `r round(mean(analytic_df$death)*100, 2)`% 

**Based on mortality between 2014-2016 for 1931-1941 birth cohort**
Number of people: `r nrow(analytic_df_birth_cohort)`
Number of observations: `r sum(as.numeric(analytic_df_birth_cohort$num_measures))`
Percent mortality: `r round(mean(analytic_df_birth_cohort$death)*100, 2)`% 

**Based on survival through age 70**
Number of people: `r nrow(analytic_df_alive_70)`
Number of observations: `r sum(as.numeric(analytic_df_alive_70$num_measures))`
Percent mortality: `r round(mean(analytic_df_alive_70$death)*100, 2)`%
