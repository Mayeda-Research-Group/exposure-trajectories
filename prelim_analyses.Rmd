...
```{r source scripts}
source(here::here("read_da_dct.R"))
```
...
```{r race-eth}
hrs_samp %<>% 
  mutate("white" = ifelse(GENDER == 2, 1, 0))
```
...
Conditional on survival to 2014, what is the association between Cystatin C and mortality after adjusting for age, gender, race/ethnicity?
...
I am using the variable [wave]CYSC_ADJ for individuals based on the recommendation in the biomarker documentation. 
