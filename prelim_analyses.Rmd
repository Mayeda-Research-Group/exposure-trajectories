...
table <- 
  ...
  mutate("Age Summary" = rep(c("2014 HRS", "All CysC")))
...
for(i in 1:length(subsets)){
  ...
  cysc_ages <- subsets[[i]] %>% dplyr::select(contains("age_y"))
...
# Loess method
ggplot(hrs_samp, aes(x = last_CYSC_age, y = OAGE)) + 
...
plot_data <- hrs_samp %>% dplyr::select(contains("AGE", ignore.case = FALSE),
                                           contains("_ADJ")) %>% 
