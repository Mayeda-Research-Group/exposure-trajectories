...
```{r seenb4 predictions}
#And are these predictions better?
MCAR_25_seenb4 <- 
  MCAR_25_test[which(MCAR_25_test$HHIDPN %in% MCAR_25_train$HHIDPN), ]

MCAR_50_seenb4 <- 
  MCAR_50_test[which(MCAR_50_test$HHIDPN %in% MCAR_50_train$HHIDPN), ]

MCAR_75_seenb4 <- 
  MCAR_75_test[which(MCAR_75_test$HHIDPN %in% MCAR_75_train$HHIDPN), ]

ggplot(aes(x = log_CYSC, y = pred_log_CYSC), 
         data = MCAR_25_seenb4) + 
    geom_point(colour = "#737C87") + 
    geom_abline(slope = 1, intercept = 0, colour = "#F8766D") + 
    ggtitle(paste0("25% Missingness")) + 
    theme_minimal()
